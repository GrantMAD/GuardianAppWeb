...
export default function RulesPage() {
  ...
  const AppSelect = ({ value, onChange }: { value: string; onChange: (v: string) => void }) => (
    <select
      ...
      className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500/50"
    >
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <p className="text-xs uppercase tracking-[0.3em] text-cyan-400 font-medium">Rules & Restrictions</p>
        <h1 className="text-3xl font-bold text-white mt-1">⚙️ Rules</h1>
        {selectedChild && (
          <p className="mt-1 text-slate-400 text-sm">Manage time limits and blocked apps for {selectedChild.name}.</p>
        )}
      </div>
      ...
      {!selectedChildId ? (
        <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/60 p-10 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">Select a child from the sidebar to manage their rules.</p>
        </div>
      ) : loading ? (
        ...
        <>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-slate-800 border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-300 hover:border-slate-600 transition-all"
            >
              🕐 Add Schedule
            </button>
          </div>
          ...
          {showLimitForm && (
            <div className="rounded-2xl border border-violet-500/30 bg-violet-500/5 p-5 space-y-4">
              ...
              <div className="flex items-center gap-3">
                <label className="text-xs text-slate-400 w-20 flex-shrink-0">Limit</label>
                <input
              ...
              <div className="flex gap-2">
                <button onClick={handleSaveLimit} disabled={savingLimit || !limitAppId}
                  className="flex-1 rounded-xl bg-violet-500 py-2 text-sm font-semibold text-white hover:bg-violet-400 disabled:opacity-50 transition-colors">
            ...
            <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-5 space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={handleSaveBlock} disabled={savingBlock || !blockAppId}
                  className="flex-1 rounded-xl bg-red-500 py-2 text-sm font-semibold text-white hover:bg-red-400 disabled:opacity-50 transition-colors">
          ...
          {showScheduleForm && (
            <div className="rounded-2xl border border-cyan-500/30 bg-cyan-500/5 p-5 space-y-4">
              <p className="text-sm font-semibold text-cyan-300">🕐 New Schedule</p>
              <input
                value={scheduleName} onChange={(e) => setScheduleName(e.target.value)}
                placeholder="Schedule name (e.g. Bedtime)"
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-cyan-500/50"
              />
              ...
              <div>
                <label className="text-xs text-slate-400 block mb-2">Days of week</label>
              ...
              <div className="flex gap-2">
                <button onClick={handleSaveSchedule} disabled={savingSchedule || !scheduleName}
                  className="flex-1 rounded-xl bg-cyan-500 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-400 disabled:opacity-50 transition-colors">
          ...
          {/* Time Limits list */}
          <div>
            <h2 className="text-xs uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-2">
              ⏳ Time Limits <span className="rounded-full bg-slate-800 px-2 py-0.5 text-slate-400">{timeLimits.length}</span>
            </h2>
            {timeLimits.length === 0 ? (
              <p className="text-center text-slate-600 text-sm py-6 rounded-2xl border border-dashed border-slate-800">No time limits set</p>
            ) : (
              <div className="space-y-2">
                {timeLimits.map((r) => (
                  <div key={r.id} className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-900 px-4 py-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-sm font-bold text-violet-400 flex-shrink-0">
                      {r.installed_apps?.app_name?.charAt(0) ?? '⏱'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white truncate">{r.installed_apps?.app_name ?? r.category ?? 'All Apps'}</p>
          ...
          <div>
            <h2 className="text-xs uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-2">
              🛑 Blocked Apps <span className="rounded-full bg-slate-800 px-2 py-0.5 text-slate-400">{blockRules.length}</span>
            </h2>
            {blockRules.length === 0 ? (
              <p className="text-center text-slate-600 text-sm py-6 rounded-2xl border border-dashed border-slate-800">No apps blocked</p>
            ) : (
              <div className="space-y-2">
                {blockRules.map((r) => (
                  <div key={r.id} className="flex items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 px-4 py-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-sm font-bold text-red-400 flex-shrink-0">
                      {r.installed_apps?.app_name?.charAt(0) ?? '🔒'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white truncate">{r.installed_apps?.app_name ?? r.category ?? 'All Apps'}</p>
          ...
          <div>
            <h2 className="text-xs uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-2">
              🗓️ Schedules <span className="rounded-full bg-slate-800 px-2 py-0.5 text-slate-400">{schedules.length}</span>
            </h2>
            {schedules.length === 0 ? (
              <p className="text-center text-slate-600 text-sm py-6 rounded-2xl border border-dashed border-slate-800">No schedules set</p>
            ) : (
              <div className="space-y-2">
                {schedules.map((s) => (
                  <div key={s.id} className={`rounded-2xl border px-4 py-3 flex items-center gap-3 ${
                    s.is_active ? 'border-cyan-500/20 bg-cyan-500/5' : 'border-slate-800 bg-slate-900'
                  }`}>
                    <div className="flex-1 min-w-0">
                    ...
                    <button onClick={() => handleToggleSchedule(s.id, s.is_active)}
                      className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ${s.is_active ? 'bg-cyan-500' : 'bg-slate-600'}`}>
